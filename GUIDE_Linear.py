...
p = 8 # Number of QAOA repetitions
path = "Graphs/"
...
def NN_opt2(net_tf, gamma, beta, opt, qcircuit_stat, step):
    # Quantum circuit samples and reshaping
    shots = 1000
    result = configs_generator(gamma, beta, configs, shots)
    NNweights = net_tf.layers[0].weights
    if step == 0:
        with tf.GradientTape() as tape:
            x = net_tf(result)
        ...
        with tf.GradientTape() as tape:
            x = net_tf(result)
            E = MaxCut_NN(G,x)
            penalty = tf.cast(tf.abs(tf.reduce_sum((NNweights[0] - tf.eye(5)))), dtype = tf.float64)
            cost = tf.reduce_sum(E)/shots + 0.1*penalty
    gradients = tape.gradient(cost, net_tf.trainable_variables)
    opt.apply_gradients(zip(gradients, net_tf.trainable_variables))
    return cost
...
for i in tqdm(range(np.shape(gammas)[0])):
    ...
    for k in range(Hyb_steps):
        ...
        NN_opt2(net_tf, gamma_hyb, beta_hyb, opt_NN, qcircuit_stat, k)

    W = net_tf.layers[0].weights
    # QAOA optimization step in original landscape. Fourth optimization step
    for k in range(QAOA2_steps):
        W0 = (1-k/QAOA2_steps)*W[0] + (k/QAOA2_steps)*tf.eye(5)
        W0_as_variable = tf.Variable(W0, name = "Weights")
        W0_as_initializer = tf.constant_initializer(W0.numpy())
        net_aNNealer = tf.keras.models.Sequential([
      tf.keras.layers.Dense(len(G.nodes), kernel_initializer= W0_as_initializer, use_bias = False),
      tf.keras.layers.Activation("tanh")
    ])
        gamma_hyb, beta_hyb, cost = QAOA_opt(gamma_hyb, beta_hyb, opt, Energies, qcircuit, Net=net_aNNealer, G=G, p=p)
